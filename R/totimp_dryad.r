...
totimp_dryad <-function(id = NA, sleep = 0, url = "http://total-impact-core.herokuapp.com/item/") {
  ...
  getdf <- function(x) {
    ...
    for(i in 1:length(x[[1]])){
      df <- rbind(
        data.frame(x[[1]][[i]]$metrics[[1]][1:3]),
        data.frame(x[[1]][[i]]$metrics[[2]][1:3]),
        data.frame(x[[1]][[i]]$metrics[[3]][1:3]),
        data.frame(x[[1]][[i]]$metrics[[4]][1:3])
        )
        # This is a comment
      df$last_update_converted <-ldply(df$last_update, unix2POSIXct)[,1]
      out_[[x[[1]][[i]]$id]] <- df
    }
    out_
  }
